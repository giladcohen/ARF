...
def get_dataset_inds(dataset: str):
    val_path = os.path.join('/tmp/adversarial_robustness', dataset, 'test_val_inds.npy')
    test_path = os.path.join('/tmp/adversarial_robustness', dataset, 'test_test_inds.npy')
    val_inds = np.load(val_path)
    test_inds = np.load(test_path)
    return val_inds, test_inds

def get_mini_dataset_inds(dataset: str):
    val_path = os.path.join('/tmp/adversarial_robustness', dataset, 'mini_test_val_inds.npy')
    test_path = os.path.join('/tmp/adversarial_robustness', dataset, 'mini_test_test_inds.npy')
    val_inds = np.load(val_path)
    test_inds = np.load(test_path)
    return val_inds, test_inds

def get_ensemble_dir(dataset: str, net: str):
    return os.path.join('/tmp/adversarial_robustness', dataset, net, 'regular')
...
def get_attack_inds(dataset, attack):
    if attack in ['fgsm', 'jsma', 'pgd2', 'deepfool', 'cw', 'cw_Linf', 'square', 'adaptive_pgd']:
        x_inds, gt_inds = 'test', 'test'
    elif attack == 'boundary':
        x_inds, gt_inds = 'mini_for_boundary', 'mini'
    elif attack in ['bpda', 'adaptive_square']:  # can be calculated for small set because we don't need validation data
    ...
    if gt_inds == 'test':
        ...
        gt_inds = mini_test_inds  # for heavy attacks: Boundary, BPDA, adaptive_square
    else:
        raise AssertionError('How did we get here?')
