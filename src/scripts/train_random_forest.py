...
FGSM, JSMA, PGD, Deepfool, CW, Square, and Boundary (only for Resnet34).
"""
import torch
...
from src.datasets.utils import get_dataset_inds, get_mini_dataset_inds, get_dump_dir, get_boundary_val_test_inds
from src.utils import set_logger, get_image_shape
...
parser.add_argument('--num_workers', default=20, type=int, help='Data loading threads for tta loader')
...
ATTACK_DIRS = ['fgsm1', 'fgsm2', 'jsma', 'pgd1', 'pgd2', 'deepfool', 'cw', 'cw_Linf', 'square', 'boundary']

if args.train_exc == 'none':
    pass
elif args.train_exc == 'fgsm':
    ATTACK_DIRS.remove('fgsm1')
    ATTACK_DIRS.remove('fgsm2')
elif args.train_exc == 'jsma':
    ATTACK_DIRS.remove('jsma')
elif args.train_exc == 'pgd':
    ATTACK_DIRS.remove('pgd1')
    ATTACK_DIRS.remove('pgd2')
elif args.train_exc == 'deepfool':
    ATTACK_DIRS.remove('deepfool')
elif args.train_exc == 'cw':
    ATTACK_DIRS.remove('cw')
    ATTACK_DIRS.remove('cw_Linf')
elif args.train_exc == 'square':
    ATTACK_DIRS.remove('square')
elif args.train_exc == 'boundary':
    ATTACK_DIRS.remove('boundary')
elif args.train_exc == 'all':
    ATTACK_DIRS = []
else:
    err = 'args.train_exc = {} is illegal'.format(args.train_exc)
    logger.error(err)
...
labels = []
for attack_dir in [''] + ATTACK_DIRS:
    if attack_dir == 'boundary':
        tmp_labels = y_gt[mini_val_inds]  # get only a mini set of the val inds
    else:
        tmp_labels = y_gt[val_inds]  # get all val inds
