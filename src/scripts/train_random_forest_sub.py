...
from src.datasets.utils import get_dataset_inds
from src.datasets.tta_logits_dataset import TTALogitsDataset
...
if os.path.exists(train_val_tta_file):
    ...
    logger.error('we cannot generate new logits, we must use the same logits that were used to train the random forest'
                 'classifier. The logits that were used to train the random forest are missing from:\n{}\n'
